...
<html>
  ...
  */
 	...
 	function doSubmit()
 	{
		...
		{
			var queryTime = document.forms[0].querytime.value;
			queryTime = queryTime.substr(0,4)+"-"+queryTime.substr(4,2);
			condition += " and substr(char(INSERTDATE),1,7) = '"+queryTime+"'";
		}
	...
	cellfunc[8]=function(datas,options)
	{
		...
		if(tmpStr[2] == '<%=loginname%>')
		{
			...
			return "<input type='button' class='form_button_short' value='删除'	onClick='deleteSingle("+tmpStr[0]+","+tmpStr[1]+")'>";
		}
		else return "";
 	}
/*
...
function saveExcel()
{
	...
	var countSql ="";
	var sql = "select OPP_NBR,ACC_NBR,NAME,manager_id,AREA,POLICE,DATE,INSERTDATE,opp_nbr||';'||acc_nbr||';'||INSERTMAN from COMPETE_OPPSTATE ";		
	sql += condition + " order by INSERTDATE desc with ur";
	return sql;
}
