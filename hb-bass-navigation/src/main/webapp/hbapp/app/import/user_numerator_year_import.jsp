 ...
 <legend>
   ...
   <table border=0 cellpadding="0" cellspacing="0">
      ...
      <td width="450"><div id="uploadDiv"></div></td>
        <td valign="top"><a style="text-decoration: underline;" href="${mvcPath}/hbirs/action/filemanage?method=downFromFtp&amp;remotePath=import&amp;fileName=分子_目标值.xls"><font color="blue"><b>数据模板(Excel)</b></font></a>&nbsp;<font color="red">请单击或右键点击“目标另存为”下载</font>
		<div>导入说明：</div>
	<div>
		<ul>
			<li></li>
		...
		+function(window,document,undefined){
			...
			function showPanel() {
				...
				vault.onAddFile = function(fileName) {
			   	...
			   	vault.setFormField("tableName", "NMK.USER_NUMERATOR_YEAR_IMPORT_TMP");
		    	vault.setFormField("columns", "TASK_ID,TYPE,AREA_CODE,IND1");
		    	vault.setFormField("date", taskId);  
		    	vault.setFormField("ds","dw");
		    	
		    	vault.onUploadComplete = function(files) {
		    		var _sql = "select count(*) cnt from NMK.USER_NUMERATOR_YEAR_IMPORT_TMP where task_id='" + taskId + "' ";
						...
						,callback : function(xmlrequest){
							try{
								...
								if(confirm("您确定要将这批数据入库吗?")){
									//删除表中数据
									var sql = encodeURIComponent("DELETE FROM NMK.USER_NUMERATOR_YEAR_IMPORT");
									sql += "&sqls=" + encodeURIComponent("INSERT INTO NMK.USER_NUMERATOR_YEAR_IMPORT SELECT TYPE,AREA_CODE,IND1 FROM NMK.USER_NUMERATOR_YEAR_IMPORT_TMP ");
									// 删除临时表的数据
									sql += "&sqls=" + encodeURIComponent("DELETE FROM NMK.USER_NUMERATOR_YEAR_IMPORT_TMP ");
			...
			function query(){
				var sql = "select * from nmk.USER_NUMERATOR_YEAR_IMPORT ";
			...
			function down(){
				aihb.AjaxHelper.down({
					sql : "select * from nmk.USER_NUMERATOR_YEAR_IMPORT with ur"
					,header : _header
					,ds : "dw"
					,isCached : false
