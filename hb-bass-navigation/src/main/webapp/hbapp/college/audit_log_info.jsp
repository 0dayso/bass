...
<%
	...
	try {
		...
		String countsql = "select count(*) from BUREAU_CFG_COLLEGE_PT_LOG where 1=1 ";
		...
		if (count > 0) {
			String sql = " select aaa.* from (select   BUREAU_ID ,  BUREAU_NAME ,  COLLEGE ,  AREA_NAME ,  DEPT_NAME , "
					...
					+ " (case when STATUS = 'IN_NEW' then '（新增）创建' when STATUS = 'OUT_NEW' then '（删除）创建' when STATUS = 'IN_ADUIT_1' then '（新增）一级审核通过' when STATUS = 'IN_ADUIT_2' then '（新增）二级审核通过' when STATUS = 'IN_ADUIT_3' then '（新增）三级审核通过' when STATUS = 'OUT_ADUIT_1' then '（删除）一级审核通过' when STATUS = 'OUT_ADUIT_2' then '（删除）二级审核通过' when STATUS = 'OUT_ADUIT_3' then '（删除）三级审核通过' "
					+ " when STATUS = 'IN_ADUIT_1_N' then '（新增）一级审核不通过' when STATUS = 'IN_ADUIT_2_N' then '（新增）二级审核不通过' when STATUS = 'IN_ADUIT_3_N' then '（新增）三级审核不通过' when STATUS = 'OUT_ADUIT_1_N' then '（删除）一级审核不通过' when STATUS = 'OUT_ADUIT_2_N' then '（删除）二级审核不通过' when STATUS = 'OUT_ADUIT_3_N' then '（删除）三级审核不通过' "
					+ " ELSE '' END),"
					+ " STATE_DATE ,  CREATEUSER ,OPERATE_TYPE,STATUS,"
					+ " row_number() over(partition by 1 ORDER BY COLLEGE,BUREAU_ID DESC) as row_num"
					+ " from BUREAU_CFG_COLLEGE_PT_LOG where 1=1";
			if (StringUtils.isNotEmpty(cityStr)) {
