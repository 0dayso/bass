...
<html xmlns:ai>
  <head>
	...
	<script type="text/javascript" src="../report/kpiSelect.js"></script>
...
var _header=[
	...
	,{"name":"描述","dataIndex":"desc","cellStyle":"grid_row_cell_text",cellFunc : abb}
	,{"name":"指标应用","dataIndex":"app_name","cellStyle":"grid_row_cell_text","cellFunc":"mapping"}
	,{"name":"唤起时间","dataIndex":"fire_date"}
	,{"name":"推送号码","dataIndex":"contacts","cellStyle":"grid_row_cell_text",cellFunc : abb}
	,{"name":"操作","dataIndex":"added1","cellStyle":"grid_row_cell","cellFunc" : function(value,options) {
			var _span = $C("span");
			var _editlink = $C("a");
			_editlink.appendChild($CT("告警任务编辑"));
			_editlink.href="javascript:void(0)";
			_editlink.onclick = function() {
				createFormPanel([
					{label : "名称：",type : "text",value : options.record.name,id : "temp_name_01"}
					,{label : "描述：",cols:45,rows:2,eleType : "textarea",value : options.record.desc,id : "temp_desc_01"}
					,{label : "",type : "hidden",value : options.record.ids,id : "temp_id_01"}
					,{label : "号码：",cols:45,rows:3,eleType : "textarea",value : options.record.contacts,id : "temp_contacts_01"}
					,{label : "",type : "button",value : "保存",className : "form_button_short"
						,onclick : function(){
							if(!confirm("确认要修改吗?"))
								return;
							var ajax = new aihb.Ajax({
								url : "${mvcPath}/hbirs/action/kpiCustomize?method=updateAuditJob"
								,parameters : "name="+encodeURIComponent($("temp_name_01").value)+"&desc="+encodeURIComponent($("temp_desc_01").value)+"&contacts="+$("temp_contacts_01").value + "&id=" + encodeURIComponent($("temp_id_01").value)
...
function query(){
	var sql = "select id as ids,name,desc,char(fire_date) fire_date,contacts,app_name,'' added1 from kpi_audit_job where user_id='"+loginname+"' with ur";
...
function process(){
  ...
  <body>
  	<div style="padding: 3px 0px 5px 6px;"><input type="button" class="form_button" value="新增告警任务" onclick="createFormPanel1()"></div>
