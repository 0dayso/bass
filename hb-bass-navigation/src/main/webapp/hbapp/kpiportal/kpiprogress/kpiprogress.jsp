...
<html>
  ...
  <script type="text/javascript">
  	...
  	var percentType = "<%=request.getParameter("percentType")%>";
	...
	function chartLink(datas,options,valuetype){
		...
		return chartLink2("1",valuetype,str);
	}
	...
	cellfunc[5] =function(datas,options){return chartLink2("2","huanbi",percentFormat(datas,options))+((chartCurZbcode in hasFluctuating)?threshold.getHuanbiImg(datas[options.seq],chartCurZbcode,kpiname,"huanbi",datas[0]):threshold.getTongbiImg(datas[options.seq],chartCurZbcode,kpiname,"huanbi",datas[0]));};
	cellfunc[6] =function(datas,options){return chartLink2("2","tongbi",percentFormat(datas,options))+((chartCurZbcode in hasFluctuating)?threshold.getHuanbiImg(datas[options.seq],chartCurZbcode,kpiname,"tongbi",datas[0]):threshold.getTongbiImg(datas[options.seq],chartCurZbcode,kpiname,"tongbi",datas[0]));};
	cellfunc[7] =function(datas,options){return chartLink2("2","yeartongbi",percentFormat(datas,options))+threshold.getTongbiImg(datas[options.seq],chartCurZbcode,kpiname,"yeartongbi",datas[0]);};
	...
	cellfunc[4] =function(datas,options){return chartLink2("2","huanbi",percentFormat(datas,options))+((chartCurZbcode in hasFluctuating)?threshold.getHuanbiImg(datas[options.seq],chartCurZbcode,kpiname,"huanbi",datas[0]):threshold.getTongbiImg(datas[options.seq],chartCurZbcode,kpiname,"huanbi",datas[0]));};
	cellfunc[5] =function(datas,options){return chartLink2("2","tongbi",percentFormat(datas,options))+threshold.getTongbiImg(datas[options.seq],chartCurZbcode,kpiname,"tongbi",datas[0]);};
							...
							if('close' == accountReturn){
								...
								var PARAM_INFO = {
									...
									if('close' == RETRUN_INFO.isClose){
										...
										alert("金库认证通过！");
										document.forms[0].action = "../action.jsp?appName="+appName+"&method=down&area="+sArea+"&date="+sDate+"&detailoffice="+detailoffice+"&zbcode="+chartCurZbcode+"&filename="+"kpiexport"+"&title="+str+"&area1="+area+"&percentType="+percentType+"&brand"+brand+"&kpiname="+kpiname;
