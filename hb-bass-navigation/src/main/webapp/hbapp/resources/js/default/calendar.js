...
//str += '<div Author="alin" class="cdrWatermark" id="cdrWatermark"></div>'
	str += "<div id=\"cdrBody\" style=\"position:absolute;left:0px;top:0px;z-index:2;width:140px;\">";
	str += this.getHeader();
	str += this.getBody();
...
Calendar.prototype.getStyle = function () {
	...
	str += ".calendar{position:absolute;width:140px!important;width /**/:142px;height:184px!important;height /**/:204px;background-color:" + this.style.bodyBackColor + ";border:1px solid " + this.style.borderColor + ";left:0px;top:0px;z-index:9999;}\n";
...
Calendar.prototype.getBody = function () {
	...
	str += "</table>\n";
	str += "<table Author=\"alin\" class=\"cdrBodyBar\" cellSpacing=\"2\" cellPadding=\"0\"><tr align=\"center\" Author=\"alin\"><td Author=\"alin\" style=\"cursor:pointer;\" onclick=\"" + this.Obj + ".getToday();\">\u4eca\u5929\uff1a" + new Date().format("yyyy\u5e74mm\u6708dd\u65e5") + "</td></tr></table>\n";
	return str;
};
...
Calendar.prototype.getYearMenu = function (year) {
	...
	for (i = 0; i < 10; i++) {
		...
		str += "onclick=\"" + this.Obj + ".bindDate('" + _date.format(this.dateFormat) + "')\">" + _year + "\u5e74</td>\n";
		str += "</tr>";
	}
...
Calendar.prototype.getMonthMenu = function () {
	...
	for (i = 1; i <= 12; i++) {
		...
		str += "onclick=\"" + this.Obj + ".bindDate('" + _date.format(this.dateFormat) + "')\">" + i + "\u6708</td></tr>\n";
	}
	str += "</table>";
	var _menu = $("cdrMenu");
	_menu.innerHTML = str;
...
Calendar.prototype.bindDate = function (date) {
	...
	var iYear = this.dateFormat.indexOf("yyyy");
	var sYear = date.substring(iYear,iYear+4);
	var iMonth = this.dateFormat.indexOf("mm");
	var sMonth = date.substring(iMonth,iMonth+2);
	var iDate = this.dateFormat.indexOf("dd");
	var sDate = date.substring(iDate,iDate+2);
	var _date = new Date(sYear, parseInt(sMonth,10) - 1, sDate);
	if (isNaN(_date)) {
		_date = new Date();
	}
	this.date = _date;
...
//获取上一个月的日期
			if (this.showMoreDay) {
				...
				_dayElement.title = _previDate.format("yyyy\u5e74mm\u6708dd\u65e5");
				_dayElement.value = _previDate.format(this.dateFormat);
...
//获取下个月的日期
				if (this.showMoreDay) {
					...
					_dayElement.title = _nextDate.format("yyyy\u5e74mm\u6708dd\u65e5");
					_dayElement.value = _nextDate.format(this.dateFormat);
					_dayElement.style.color = this.style.moreDayColor;
					_nextDay++;
				} else {
					_dayElement.innerHTML = "";
...
//获取本月日期
					...
					_dayElement.title = _curDate.format("yyyy\u5e74mm\u6708dd\u65e5");
					_dayElement.value = _curDate.format(this.dateFormat);
					_day++;
				} else {
					_dayElement.innerHTML = "";
					_dayElement.title = "";
...
//var _watermark = $("cdrWatermark");
	_curYear.innerHTML = this.date.format("yyyy\u5e74");
	_curMonth.innerHTML = this.date.format("mm\u6708");
//_watermark.innerHTML = this.date.getFullYear();     
};
Calendar.prototype.getToday = function () {
	var _date = new Date();
	this.bindDate(_date.format(this.dateFormat));
};
...
Calendar.prototype.onClick = function (obj) {
	...
	if(this.callback)this.callback(obj.value);
	this.hide();
};
...
Date.prototype.format = function (fs) {
	...
	if(_date<10)_date="0"+_date;
	if (fs.length == 1) {
		return this.getFullYear() + fs + _month + fs + _date;
	}
	fs = fs.replace("yyyy", this.getFullYear());
	fs = fs.replace("mm", _month);
...
function closeCalendar(evt) {
	...
	if (!_target.getAttribute("Author") && _target != inputObj && _target != targetObj && $("Calendar")) {
