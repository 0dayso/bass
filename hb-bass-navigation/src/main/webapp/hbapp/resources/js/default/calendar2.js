...
function WebCalendar() //初始化日历的设置
{
    ...
    //add by wentn
    this.rtnContent = "DAY";           //返回的内容 年，月，日
...
function calendar() //主调函数
{
    ...
    try{
        if (WebCalendar.objExport.value.trim() != ""){
            if(WebCalendar.rtnContent=="MONTH"){
                var tmpInputData = "";
                tmpInputData += WebCalendar.objExport.value.trim()+"-01";
                WebCalendar.dateStyle = tmpInputData.match(WebCalendar.dateReg);
            }
            else if(WebCalendar.rtnContent=="DAY"){
