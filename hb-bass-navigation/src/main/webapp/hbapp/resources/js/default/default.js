		...
		for(var j=0;j<_headerName.length;j++){
			...
			for(var i=0; i<_header.length; i++ ){
				...
				if(_element.span && _element.display){
					...
					//火狐不支持event
					if((typeof event != 'undefined')&&event.srcElement&&event.srcElement.id&&event.srcElement.id==_header[i].dataIndex){
						if(_grid.order=="asc"){
							_a.innerText+="↑";
						}else{
							_a.innerText+="↓";
...
aihb.AjaxGrid.prototype = {
	...
	initialize : function(options){
		...
		var ajax = new aihb.Ajax({
			...
			,parameters : "start="+obj.start+"&limit="+obj.limit+"&sql="+strEncode(_sql)+"&"+_params._oriUri + (options.ds?"&ds="+options.ds:"") + (options.isCached!=undefined?"&isCached="+options.isCached:"")
			,loadmask : true
			,callback : function(xmlrequest){
...
aihb.AjaxHelper={
	...
	*/
	...
	down : function( options ){//form:表单, sql：sql语句,header:表头,fileName:下载的文件名,url:请求下载的地址,ds:数据源,useExcel:是否下载excel
		...
		var _url = options.url || "/hb-bass-navigation/hbirs/action/jsondata?method=down";
