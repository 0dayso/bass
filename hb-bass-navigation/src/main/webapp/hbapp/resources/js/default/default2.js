...
var $N = function(_name){return document.getElementsByName(_name);}
...
aihb.AjaxGrid.prototype = {
	...
	initialize : function(options){
		...
		var ajax = new aihb.Ajax({
			url : "/hb-bass-navigation/hbirs/action/jsondata?method=query&qType=limit"
			,parameters : "start="+obj.start+"&limit="+obj.limit+"&sql="+encodeURIComponent(_sql)+"&"+_params._oriUri + (options.ds?"&ds="+options.ds:"") + (options.isCached!=undefined?"&isCached="+options.isCached:"")
			,loadmask : true
			,callback : function(xmlrequest){
...
aihb.SimpleGrid.prototype = {
	...
	initialize : function(options){
		...
		var ajax = new aihb.Ajax({
			...
			,parameters : "sql="+encodeURIComponent(_sql)+"&"+_params._oriUri + (options.ds?"&ds="+options.ds:"") + (options.isCached!=undefined?"&isCached="+options.isCached:"")
			,loadmask : true
			,callback : function(xmlrequest){
				var _elObj=$(options.el||"grid");
				_elObj.style.display="";
...
aihb.PieceGrid.prototype = {
		initialize : function(options){
			...
			var ajax = new aihb.Ajax({
				...
				,parameters : "sql="+encodeURIComponent(_sql)+"&rows="+_rows+"&"+_params._oriUri + (options.ds?"&ds="+options.ds:"") + (options.isCached!=undefined?"&isCached="+options.isCached:"")
				,loadmask : true
				,callback : function(xmlrequest){
					var _elObj=$(options.el||"grid");
					_elObj.innerHTML="";
...
aihb.FormHelper = {
	...
	fillSelectWrapper : function(options){
		...
		var ajax = new aihb.Ajax({
			...
			,parameters : "sql="+encodeURIComponent(options.sql)+(options.ds?"&ds="+options.ds:"")+"&"+_params._oriUri + (options.isCached!=undefined?"&isCached="+options.isCached:"")
			,callback : function(xmlrequest){
