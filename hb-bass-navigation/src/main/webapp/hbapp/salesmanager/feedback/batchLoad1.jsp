...
<%String loginname = (String)session.getAttribute("loginname");
	...
	System.out.println("loginname="+loginname+"   areaId="+areaId);
%>
<html xmlns:ai>
<HEAD>
...
//	var _params = aihb.Util.paramsObj();
	...
	function showPanel() {
    	...
    	sql += "&sqls="+encodeURIComponent("update COMPETE_OPPSTATE_UPLOAD set flag='导入移动号码相同' where ACC_NBR in (select ACC_NBR from COMPETE_OPPSTATE_UPLOAD where task_id='" + task_id + "' group by ACC_NBR having count(*)>1)");
			...
			sql += "&sqls=" + encodeURIComponent("insert into COMPETE_OPPSTATE (OPP_NBR,ACC_NBR,NAME,MANAGER_ID,AREA,POLICE,DATE,STATE,INSERTMAN,city_id) select distinct OPP_NBR,ACC_NBR,NAME,MANAGER_ID,AREA,POLICE,date(DATE),'1','<%=loginname%>','"+aihb.Constants.getArea('<%=areaId%>').cityCode+"' from COMPETE_OPPSTATE_UPLOAD where task_id='" + task_id + "' and flag='成功'");
 ...
 <legend>
   ...
   <table border=0 cellpadding="0" cellspacing="0">
      <tr style="padding:5px;"> 
        ...
        <td valign="top"><a style="text-decoration: underline;" href="/hbbass/common/FileDown.jsp?filename=sample.xls&filepath=/hbbass/salesmanager/feedback/"><font color=blue><b>导入数据模板(Excel)</b></font></a>&nbsp;<font color="red">请右键点击“目标另存为”下载</font>
        	<div>导入说明：</div>
