...
<input type="hidden" name="nextPage2" value="<%=nextPage%>">
  ...
  <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="page">
    <tr> 
      ...
      <td width="33%"> 
        <div align="center">共<%=Divpage.getAllRowCount()%> 条，列出第<%=Divpage.getBeginRow()%>到第<%=Divpage.getEndRow()%>条</div>
            </td>
    <td width="26%"> 
      <div align="center"> 
        ...
        上页 
        <% } else { %>
        <a href='javascript:nextpage(<%=Divpage.getNextPage()-2%>)'>上页</a> 
        <% } %>
        <% if (Divpage.getIfNext()){ %>
        <a href='javascript:nextpage(<%=Divpage.getNextPage() %>)'>下页</a> 
        <%}else{%>
        下页 
        <%}%>
        <% if (Divpage.getMaxPage() <= 1){ %>
        尾页 
        <% } else { %>
        <a href='javascript:nextpage(<%=Divpage.getMaxPage()-1%>)'>尾页</a> 
        <%}%>
      </div>
            </td>
            
      <td width="18%"><div align="center">第<%=Divpage.getNextPage()%>/<%=Divpage.getMaxPage()%>页</div></td>
