...
function kpiCompare(datas,options)
{
	var seq = options.seq;
	var columnoffset = 2;
	var areaCode= datas[0];
	var textName=document.getElementById("selectcolumns_div").getElementsByTagName("input")[seq].parentNode.childNodes[1].nodeValue;
	return "<a href=kpichart.jsp?tablename="+tablescons+"&conditions="+encodeURIComponent(conditionscons)+"&column="+encodeURIComponent(arrcolumns[seq-columnoffset])+"&name="+encodeURIComponent(mappingArea(datas,{seq:0})+" "+textName)+"&area_code="+areaCode+" target=_blank>"+numberFormatDigit2(datas,{seq:seq})+"</a>";
}

function loseuser(datas,options)
{
	var value = datas[1].split(" ");
	if(value.length > 1)return "<a href='loseuserdetail.jsp?month="+document.forms[0].month.value+"&groupcode="+value[0]+"' title='�鿴������ϸ' target='_blank'>"+percentFormat(datas,options)+"</a>"
...
{
	var value = datas[1].split(" ");
	if(value.length > 1) return "<a href='incomedetail.jsp?month="+document.forms[0].month.value+"&groupcode="+value[0]+"' title='�鿴������ϸ' target='_blank'>"+numberFormatDigit2(datas,options)+"</a>"
	else return numberFormatDigit2(datas,options);			
}
...
{
	var value = datas[1].split(" ");
	if(value.length > 1) return "<a href='memberdetailreqring.jsp?month="+document.forms[0].month.value+"&groupcode="+value[0]+"' title='�鿴��Ա��Ϣ' target='_blank'>"+numberFormatDigit2(datas,options)+"</a>"
	else return numberFormatDigit2(datas,options);	
}
...
{
	var value = datas[1].split(" ");
	if(value.length > 1) return "<a href='memberdetailreqsms.jsp?month="+document.forms[0].month.value+"&groupcode="+value[0]+"' title='�鿴��Ա��Ϣ' target='_blank'>"+numberFormatDigit2(datas,options)+"</a>"
	else return numberFormatDigit2(datas,options);
}
...
{
	var value = datas[1].split(" ");
	if(value.length > 1) return "<a href='memberdetailreqmms.jsp?month="+document.forms[0].month.value+"&groupcode="+value[0]+"' title='�鿴��Ա��Ϣ' target='_blank'>"+numberFormatDigit2(datas,options)+"</a>"
	else return numberFormatDigit2(datas,options);
}
...
{
	var value = datas[1].split(" ");
	if(value.length > 1) return "<a href='memberdetailreqgprs.jsp?month="+document.forms[0].month.value+"&groupcode="+value[0]+"' title='�鿴��Ա��Ϣ' target='_blank'>"+numberFormatDigit2(datas,options)+"</a>"
	else return numberFormatDigit2(datas,options);
}

function resAlert(datas,options){return datas[options.seq-1]<0.2?"<font color='green'>��</font>":"<font color='red'>�쳣</font>"}
...
function groupcustRenderChart(spelltype,columns,tables,condition,group,postfix,chartcolumn)
{
	...
	if(tag=="city")
	{
		...
		{
			tag="county";
			column = "value(substr(channel_code,1,8),'����')";
		}
	}
	arr = groupcustBaseSpellSQL(spelltype,columns,tables,condition,group,postfix);
	renderChart("tagname="+encodeURIComponent(tag)+"&sql="+encodeURIComponent(arr[2].replace(/#{value}/gi,"value("+column+",'����')"))+"&name="+encodeURIComponent(chartName));
}
...
function channelTerminalQuery(spelltype,funcSubmit,columns,tables,condition,group,postfix)
{
	arr = channelTerminalSpellSQL(spelltype,columns,tables,condition,group,postfix);
	if(arr[3] != undefined)arr[3](arr[0],arr[1]); //ִ�в�ѯ��ʾ
	else funcSubmit(arr[0],arr[1]);
}
...
function channelBaseSpellSQL(spelltype,columns,tables,condition,group,postfix)
{
	...
	if(channelName && document.forms[0].channeltype!=undefined && document.forms[0].channelkind!=undefined)
	{
		...
		if(channel3.value!="")
		{
			...
			columnPlaceholder += ",'"+mapping(document.forms[0].channelkind.value,"channelkind") + "','"+ mapping(channel3.value,"channeltype")+"',case when grouping(a.channel_name)=1 then '�ϼ�' else value(a.channel_name,'����') end";
		...
		else if(document.forms[0].channelkind.value!="")
		{
			if(spelltype=="normal")
			{
				...
				columnPlaceholder += ",'"+mapping(document.forms[0].channelkind.value,"channelkind") + "',case when grouping("+ channel3.parentNode.name + ")=1 then '�ϼ�' else value("+ channel3.parentNode.name + ",'����') end,'--'";
			}
		...
		else
		{
			if(spelltype=="normal")
			{
				...
				columnPlaceholder += ",case when grouping("+ document.forms[0].channelkind.parentNode.name + ")=1 then '�ϼ�' else value("+ document.forms[0].channelkind.parentNode.name +  ",'��������') end,'--','--'";
			}
			else if (spelltype=="sort")
			{
				columnPlaceholder +=",'ȫ��','--','--'";
	...
	else if( channelName && (document.forms[0].channeltype!=undefined || document.forms[0].officetype!=undefined || document.forms[0].sochannel!=undefined || document.forms[0].echannel!=undefined ))
	{
		...
		if(channel2.value!="")
		{
			...
			tables = "(select channel_id as ch_id,channel_name from nmk.RES_SITE_DS) a,"+tables;
			group += "," + "a.channel_name";
			columnPlaceholder += ",'" + mapping(channel2.value,channelTagName) + "',case when grouping(a.channel_name)=1 then '�ϼ�' else value(a.channel_name,'����') end";
			selectColumns(2+areaOffset,true);
		}
		else if(channel2.value=="")
		{
			if(spelltype=="normal")
			{
				...
				columnPlaceholder += ",case when grouping("+channel2.parentNode.name + ")=1 then '�ϼ�' else value("+channel2.parentNode.name + ",'����') end,'--'";
	...
	else if(document.forms[0].channelkind!=undefined)
	{
		...
		else
		{
			if(spelltype=="normal")
			{
				group += ",case when grouping("+document.forms[0].channelkind.parentNode.name + ")=1 then '�ϼ�' else value("+document.forms[0].channelkind.parentNode.name + ",'����') end";
	...
	else if(document.forms[0].channeltype!=undefined || document.forms[0].officetype!=undefined || document.forms[0].sochannel!=undefined || document.forms[0].echannel!=undefined|| document.forms[0].xpofficetype!=undefined)
	{
		seq=1;
		channelTagName = "channeltype";
		...
		else if(document.forms[0].xpofficetype!=undefined){channel1 = document.forms[0].xpofficetype;channelTagName="xpofficetype";}
		if(channel1.value!="")
		{
		...
		else
		{
			if(spelltype=="normal")
			{
				...
				columnPlaceholder += ",case when grouping("+channel1.parentNode.name + ")=1 then '�ϼ�' else value("+channel1.parentNode.name + ",'����') end";
				//cellfunc[1+areaOffset]=function(datas,options){return mapping(datas[options.seq],channelTagName);};
			}
			else if (spelltype=="sort")
			{
...
function channelTerminalSpellSQL(spelltype,columns,tables,condition,group,postfix)
{
	var sql = "select #{columns} from #{tables} where #{condition} group by #{group} #{order} #{postfix} with ur";
	...
	//���⴦�����
	if(document.forms[0].terminaltype==undefined&&document.forms[0].terminalbrand==undefined)
	{
		if(document.forms[0].office!=undefined && document.forms[0].office.value!="")
		{
			columnPlaceholder += "'"+mapping(document.forms[0].office.value,"office")+"'";
			if(condition.length>0)condition += " and ";
			group = document.forms[0].office.parentNode.name+ group;
			condition += document.forms[0].office.parentNode.name + "='" + document.forms[0].office.value + "'";
		}
		else if(document.forms[0].county!=undefined && document.forms[0].county.value!="")
		{
			columnPlaceholder += document.forms[0].office.parentNode.name;
			group = document.forms[0].office.parentNode.name+ group;
			if(condition.length>0)condition += " and ";
			condition += document.forms[0].county.parentNode.name + "='" + document.forms[0].county.value + "'";
		}
		...
		{
			columnPlaceholder += document.forms[0].county.parentNode.name;
			group = document.forms[0].county.parentNode.name+ group;
			if(condition.length>0)condition += " and ";
			condition += document.forms[0].city.parentNode.name + "='" + document.forms[0].city.value + "'";
		}
		else
		{
			columnPlaceholder += document.forms[0].city.parentNode.name;
			group = document.forms[0].city.parentNode.name+ group;
		}
	}
	else if(document.forms[0].terminaltype==undefined&&document.forms[0].terminalbrand!=undefined)
	{
		if(document.forms[0].office!=undefined && document.forms[0].office.value!="")
		{
			columnPlaceholder += "'"+mapping(document.forms[0].office.value,"office")+"'";
		...
		//���⴦���ն�
		columnPlaceholder+=",value((select tac_pro from nwh.imei_tacpro_info where tac_pro_id= a.term_brand),'����')";
		group = " a.term_brand, " + group;
	}
	else
	{
		if(document.forms[0].office!=undefined && document.forms[0].office.value!="")
		...
		{
			columnPlaceholder += "'"+mappingArea(document.forms[0].county.value)+"'";
			if(condition.length>0)condition += " and ";
			condition += document.forms[0].county.parentNode.name + "='" + document.forms[0].county.value + "'";
		}
		else if(document.forms[0].city!=undefined && document.forms[0].city.value!="" && document.forms[0].city.value!="0")
		{
			columnPlaceholder += "'"+mappingArea(document.forms[0].city.value)+"'";
			if(condition.length>0)condition += " and ";
			condition += document.forms[0].city.parentNode.name + "='" + document.forms[0].city.value + "'";
		}
		else
		{
			columnPlaceholder += "'ȫʡ'";
		}
		//���⴦���ն�
		columnPlaceholder+=",value((select tac_pro from nwh.imei_tacpro_info where tac_pro_id= term_brand),'����'),value((select device_name from nwh.imei_terminal_info where device_id= term_model),'����')";
		group = " term_brand,term_model " + group;
	}
	...
	for(var i=0; i < attribute.length; i++)
	{
					...
					&& attribute[i].childNodes[0].name != "county")
		{
			...
			else if(attribute[i].type=="terminal")
			{
				if(attribute[i].name=="term_brand")condition += attribute[i].name + " in (select tac_pro_id from nwh.imei_tacpro_info where tac_pro like '%" + attribute[i].childNodes[0].value + "%')";
				else condition += attribute[i].name + " in (select device_id from nwh.imei_terminal_info where device_name like '%" + attribute[i].childNodes[0].value + "%')";
			}
			else 		condition += attribute[i].name + "='" + attribute[i].childNodes[0].value + "'";
		}
		if(attribute[i].name=="op_time")optimeValue=attribute[i].childNodes[0].value;
	...
	sql=sql.replace("#{tables}",tables).replace(/#{condition}/gi,condition);
	
	chartsql = chartsql.replace("#{tables}",tables).replace(/#{condition}/gi,condition);
	
	if(condition.length > 0)chartsql = chartsql.replace("#{condition}",condition);
	
	var countSql= sql.replace("#{columns}","count(*)").replace("#{postfix}","").replace("group by #{group}","").replace("#{order}","");//���ͳ������SQL
	
	sql = sql.replace("#{columns}",columns.replace(/#{condition}/gi,condition));
	
	document.getElementById("sql").value=sql.replace("#{postfix}","").replace("#{group}",group).replace("#{order}","");//���������SQL
	
	if(spelltype=="normal")
	{
		sql = sql.replace("#{order}"," order by ordercolumn desc").replace("#{group}",group).replace("#{postfix}",postfix);//���Ͻ�ȡ��׺
	}
	else if (spelltype=="sort")
	{
		sql = sql.replace("#{order}","order by ordercolumn " + document.forms[0].order_desc.value).replace("#{group}",""+group+"").replace("#{postfix}",postfix);//���Ͻ�ȡ��׺
	}
...
function groupcustBaseSpellSQL(spelltype,columns,tables,condition,group,postfix)
{
	...
	if(spelltype=="normal")
	{
		if(document.forms[0].county!=undefined && document.forms[0].county.value!="")
		{
			...
			columnPlaceholder += "value(STAFF_NAME,'����'),case when grouping(groupcode||' '||groupname)=1 then '�ϼ�' else value(groupcode||' '||groupname,'����') end";
			if(condition.length>0)condition +=" and ";
			condition += document.forms[0].county.parentNode.name + "='" + document.forms[0].county.value + "'";
		}
		else if(document.forms[0].city!=undefined && document.forms[0].city.value!="" && document.forms[0].city.value!="0")
		{
			...
			columnPlaceholder += document.forms[0].county.parentNode.name+",case when grouping(STAFF_NAME )=1 then '�ϼ�' else value(STAFF_NAME,'����') end";
			if(condition.length>0)condition +=" and ";
			condition += document.forms[0].city.parentNode.name + "='" + document.forms[0].city.value + "'";
		}
		else if(document.forms[0].city!=undefined && document.forms[0].city.value!=undefined)
		{
			...
			orderby = group;
			columnPlaceholder += document.forms[0].city.parentNode.name+",case when grouping("+document.forms[0].county.parentNode.name+")=1 then '�ϼ�' else value("+document.forms[0].county.parentNode.name+",'����') end";
		}
	}
...
function groupcustSpellExtemporeSQL(columns,tables,condition,group,postfix)
{
	...
	if(document.forms[0].order!=undefined && document.forms[0].order.value!="")sql = sql.replace("#{order}"," order by " + document.forms[0].order.value +" desc ");
...
function groupcustGroupQuery(columns,tables,condition,group,postfix)
{
	arr = groupcustSpellGroupSQL(columns,tables,condition,group,postfix);
	if(arr[3] != undefined)arr[3](arr[0],arr[1]); //ִ�в�ѯ��ʾ
	else ajaxSubmitWrapper(arr[0],arr[1]);
}

function groupcustSpellGroupSQL(columns,tables,condition,group,postfix)
{
	var sql = "select #{columns} from #{tables} where #{condition} #{group} #{order} #{postfix} with ur";
	...
	for(var i=0; i < attribute.length; i++)
	{
							...
							&& attribute[i].name != "monthcons" )
		{
			...
			else if(attribute[i].type=="threshold")condition += attribute[i].name + ">=" + attribute[i].childNodes[0].value;
			else if(attribute[i].type=="int")	condition += attribute[i].name + "=" + attribute[i].childNodes[0].value;
			else if(attribute[i].type=="like")condition += attribute[i].name+ " like '%" + attribute[i].childNodes[0].value + "%'";
			else if(attribute[i].type=="in")	condition += attribute[i].name + " in (" + attribute[i].childNodes[0].value + ")";
			else if(attribute[i].type=="varchar")condition += attribute[i].name + "='" + attribute[i].childNodes[0].value + "'";
		...
		if(attribute[i].grouping!=undefined)
		{
			//if(attribute[i].name=="monthcons")channelplaceholder = "'"+attribute[i].childNodes[0].value+"',"+channelplaceholder;
			//else channelplaceholder += ","+attribute[i].name;
			if(channelplaceholder.length>0)channelplaceholder+=",";
			channelplaceholder += attribute[i].name;
			if(group.length>0)group+=",";
			group = attribute[i].name;
		}
	}
	columns = channelplaceholder + "," +columns;
	...
	var countSql= sql.replace("#{columns}","count(*)").replace("#{postfix}","").replace("#{group}","").replace("#{order}","");//���ͳ������SQL
	
	sql = sql.replace("#{columns}",columns).replace("#{order}"," order by 1").replace("#{group}"," group by "+group);
