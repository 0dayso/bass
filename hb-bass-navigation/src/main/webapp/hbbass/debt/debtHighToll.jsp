...
<HEAD>
	...
	<TITLE>国际长途(欠费100元以上)高额欠费用户</TITLE>
...
<fieldset>
    ...
    <table id="resultTable" align="center" width="99%" class="grid-tab-blue" cellspacing="1" cellpadding="0" border="0">
		<tr class="grid_title_blue">
		   ...
		   <span id=4><td class="grid_title_blue_noimage" title="高额短信用户欠费收入/总收入"><a href='#' onclick="doSubmit(this)">收入占比<ai:piece></ai:piece></a></td></span>
		   <span id=5><td class="grid_title_blue_noimage" title="高额短信用户欠费/总欠费"><a href='#' onclick="doSubmit(this)">欠费占比<ai:piece></ai:piece></a></td></span>
		   <span id=6><td class="grid_title_blue_noimage" title="高额短信欠费用户数较上月同期的增幅"><a href='#' onclick="doSubmit(this)">增幅<ai:piece></ai:piece></a></td></span>
...
<script type="text/javascript">
	...
	cellfunc[3] = function(datas,options){
		return "<a href='#' title='国际长途高额欠费用户清单' onclick=add('debtHighTollDetail','国际长途高额欠费用户清单','debtHighTollDetail.jsp')>"+numberFormatDigit2(datas,options)+"</a>"
	}
	...
	function doSubmit()
	{
		...
		var sql=" select "+placeholder
			...
			+" ,sum(case when etl_cycle_id=%date% and highinter_tid=1 then month_debtcharge_users end) cur_totuser_h"
			+" ,sum(case when etl_cycle_id=%date% and highinter_tid=1 then month_debtcharge end)  cur_totchg_h"
			+" ,sum(case when etl_cycle_id=%date% and highinter_tid=1 then bill_charge end)  cur_totbchg_h"
			...
			+" ,sum(case when etl_cycle_id=%date1% and highinter_tid=1 then month_debtcharge_users end) pre_totuser_h"
			+" ,sum(case when etl_cycle_id=%date1% and highinter_tid=1 then month_debtcharge end)  pre_totchg_h"
			+" ,sum(case when etl_cycle_id=%date1% and highinter_tid=1 then bill_charge end)  pre_totbchg_h"
