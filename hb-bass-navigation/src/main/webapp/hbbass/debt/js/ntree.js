        ...
        this.expandObj.onclick = function()
        {
			 ...
			 //if(tid == "21" || tid == "23")
			 {
				if(self.imgBar3.style.display == "none"){
					...
					while(oNode.infoField)
					{
						if(new RegExp("mcust_Lid","gi").test(oNode.infoField.childNodes[0].dim)
						||new RegExp("nbilling_tid","gi").test(oNode.infoField.childNodes[0].dim)){
							if(sCond.length>0)sCond += " and ";
							sCond += oNode.infoField.childNodes[0].dim;
						}else if(new RegExp("biz_type","gi").test(oNode.infoField.childNodes[0].dim)){
							bizTypeFlag=true;
							bizTypeName=oNode.infoField.childNodes[0].dim.split("=")[1];
						}
						oNode = oNode.parent;
					}
					
					if(bizTypeFlag)zbcode=bizType[bizTypeName];
					else zbcode = orizbcode;
					condition=sCond;
					
					//显示选择dim对话框
					showTip(aa.selectedNode.infoField.childNodes[0]); 
					//return ;	
        		}
        	}
...
function initTable(name){
	indexCount = 0 ;
 	table = ""
		  + "<table id='0' name='"+name+"' border='0' dim='' cellspacing='0' cellpadding='0'>"
		  + "	<tr>"
		  + "		<td>"
		  + "		<table background='./img/imc_11.gif' border='0' cellpadding='0' cellspacing='0' width='150'>"
		  + "			<tr>"
...
function showTip(obj) {
	...
	if(curConfDim.length==0){
		var id = obj.id+"";
		if(id!="0" && id.length==1){
			content +="	<div><input type='radio' value='2' name='R1' onclick=getOption(this.value,'abc','"+obj.name+"');>业务结构</div>";
		}else if (id.length==2){
		}
		else{
			content +="	<div><input type='radio' value='BIZ_TYPE' name='R1' onclick=getOption(this.value,'abc');>业务结构</div>"
	...
	function drawTip(tipName,tipContent){
  		...
  		if(tipObj){
    		...
    		tipObj.style.left = (mousex+pagexoff) + 5;
 			tipObj.style.top = (mousey+pageyoff) - 25;
