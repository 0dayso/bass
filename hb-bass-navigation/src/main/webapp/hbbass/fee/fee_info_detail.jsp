...
function genSQL(){
	var sql="select FEE_ID c1 ,FEE_RATE_TYPE c2 ,FEE_RATE_TYPE_NAME c3 ,FEE_RATES c4 ,FEE_DISCOUNT_DURA c5 ,FEE_DISCOUNT_CHARGE c6 ,RENT_CHARGE c7 ,MINIMUM_CHARGE c8,case when dura_flag=1 then '包含在优惠中' else '不包含在优惠中' end c10,case when charge_flag=1 then '包含在优惠中' else '不包含在优惠中' end c11,maxmum_charge c12 from NWH.FEE_RATE_DISCOUNT_INFO where fee_id = '<%=fee_id%>' "; 
						...
						<TABLE class=grid-tab-blue cellSpacing=1 cellPadding=0 width=99% align=center border=0>
							...
							for(int i = 0 ;i<list.size();i++){
								...
								<TR class=grid_row_blue>
									...
									<TD class=grid_row_cell width="40%"><%=arr[2] %></TD>
...
function update(){
	tabAdd({url:'/hbbass/fee/fee_info_update.jsp?fee_id=<%=fee_id%>',title:'修改资费'});
}
</script>