...
var _header=[
	...
	,{"name":"主网格编码","dataIndex":"c1"}
	,{"name":"主网格名称","dataIndex":"c2"}
	,{"name":"主网格类型","dataIndex":"c3"}
	//,{"name":"地市编码","dataIndex":"c4"}
	,{"name":"地市","dataIndex":"c5"}
	//,{"name":"级别","dataIndex":"c6"}
	,{"name":"网格描述","dataIndex":"c7"}
];
...
if($('zwg').value != ""){
	wherePart +=	" and maingrid_name like '%"+$('zwg').value+"%'";
}
if($('wglx').value != ""){
...
<%
String idPara = request.getParameter("maingrid_id");
%>
if(<%=idPara%> && <%=idPara%> != '' && treeOpened == false){
	wherePart +=	" and maingrid_id = <%= idPara%> ";
	treeOpened = true;
}
	
var sql= "select '' c0,maingrid_id c1,maingrid_name c2,case when type='ADMIN' then '物理主网格' when type='SPE' then '特征主网格' end c3,city_id c4,city_name c5,level c6,remark c7 from nmk.grid_main_info"
		+ wherePart
		+" order by c1 "
...
function del(){
	...
	document.forms(0).action='${mvcPath}/hbirs/action/entGrid?method=deleteGridMainInfo&maingrid_id='+ids;
	document.forms(0).submit();
}

function add(){
	window.location = '/hbbass/grid/grid_main_info_add.jsp';
}
...
function update(){
	...
	document.forms(0).action='${mvcPath}/hbirs/action/entGrid?method=getGridMainInfo&maingrid_id='+ids;
	document.forms(0).submit();
}
