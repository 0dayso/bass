...
<HTML>                                                                                                                                                                                                                                                                                                                   
<HEAD>
<TITLE>湖北移动经营分析系统-标杆集团客户清单导入</TITLE>
 ...
 <legend>
   ...
   <table border=0 cellpadding="0" cellspacing="0">
      ...
      <td width="450"><div id="uploadDiv"></div></td>
        <td valign="top"><a style="text-decoration: underline;" href="${mvcPath}/hbirs/action/filemanage?method=downFromFtp&amp;remotePath=import&amp;fileName=标杆集团数据.xls"><font color="blue"><b>数据模板(Excel)</b></font></a>&nbsp;<font color="red">请单击或右键点击“目标另存为”下载</font>
			<div>说明：</div>
...
<fieldset>
	...
	function showPanel() {
		...
		vault.setFormField("tableName", "nmk.dw_ent_import2_temp");
		vault.setFormField("columns", "task_id, sequence_id, o_area_name , o_group_name, groupcode, info_ext1");
		vault.setFormField("date", taskId);
		vault.setFormField("ds","dw");
		vault.onUploadComplete = function(files) {
			var _sql = "select count(*) cnt from nmk.dw_ent_import2_temp where task_id='" + taskId +"' ";
			var _ajax = new aihb.Ajax({
				...
				try{
						...
						if(confirm("您确定要将这批数据入库吗?")){
							var sql = encodeURIComponent("delete from nmk.dw_ent_import2");
							sql += "&sqls=" + encodeURIComponent("insert into nmk.dw_ent_import2(sequence_id, o_area_name , o_group_name, groupcode, info_ext1, info_ext2, info_ext3, info_ext4) select sequence_id, o_area_name , o_group_name, groupcode, info_ext1, info_ext2, info_ext3, info_ext4 FROM nmk.dw_ent_import2_temp ");
										// 删除临时表的数据
										sql += "&sqls=" + encodeURIComponent("delete from nmk.dw_ent_import2_temp ");
	...
	,{"name":"原集团单位名称","dataIndex":"o_group_name","cellStyle":"grid_row_cell_text"}
	,{"name":"原集团单位ID","dataIndex":"groupcode","cellStyle":"grid_row_cell_text"}
	,{"name":"2012年集团客户信息化收入目标（万元）","dataIndex":"info_ext1","cellStyle":"grid_row_cell_text"}
	];	
	
	function genSQL() {
		var sql = "SELECT sequence_id, o_area_name , o_group_name, groupcode, info_ext1, info_ext2, info_ext3, info_ext4  FROM nmk.dw_ent_import2 WHERE 1= 1 order by 1 ";
		return sql;
	}
