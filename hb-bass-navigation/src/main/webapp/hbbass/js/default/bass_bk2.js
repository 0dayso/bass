...
function createTab() {
	...
	this.createBody = function() {
		...
		for(i=0;i<cTab.tabLen;i++)
		{
			...
			if(tabAttriable[3])
			{
				...
				tabHTML += "<img src=\""+path+"images/default/" + tabIcon + "\" border=\"0\" align=\"absmiddle\" style=\"margin-right:2px;\" />\n";
			}
			tabHTML += tabTitle + "</td>\n";

			if(i>=this.fixNo)
			{
				tabHTML += "<td class=\""+tabTextClassOff+"\" valign=\"top\" style=\"padding:8px 2px 0 px 0px;cursor:hand;\" id=\"tdCloseTab"+(i+1)+"\"><img src=\""+path+"images/default/icon_closetab.gif\" name=\"iconCloseTab"+(i+1)+"\" id=\"iconCloseTab"+(i+1)+"\" onclick=\"theCreateTab.subtract("+i+")\" border=\"0\" /></td>\n";
			}
	...
	this.createIfra = function() {
		...
		for(i=0;i<ifraStr.length;i++)
		{
			...
			//var sHTML = '<iframe border=0 src="'+ifraStrs[2]+'" scrolling = "auto" width="100%" height="100%" frameborder="0" ></iframe>';
