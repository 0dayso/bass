		...
		//document.getElementById("sqlview").innerHTML+= "MOU进入" + "\n";
			...
			sql_value = "DECIMAL(FLOAT(SUM(SUM_DURA/60))/" + 
	   				"(SELECT SUM(USER_NUMS) FROM NMK.ST_MZONE_CONSUME_INFO_MM " + 
						"WHERE CITY_ID= SUBSTR(A.AREA_ID,1,5) AND " + 
							  "OP_TIME='" + _time_id + "' " + 
						"GROUP BY  CITY_ID" + 
						") " + 
				",10,3)";
			} else if("county" == sql_dim_table) {
				//document.getElementById("sqlview").innerHTML+= "情况2" + "\n";
				sql_value = "DECIMAL(FLOAT(SUM(SUM_DURA/60))/" + 
	   				"(SELECT SUM(USER_NUMS) FROM NMK.ST_MZONE_CONSUME_INFO_MM " + 
						"WHERE COUNTY_ID= SUBSTR(A.AREA_ID,1,8) AND " + 
							  "OP_TIME='" + _time_id + "' " + 
						"GROUP BY  COUNTY_ID" + 
						")" +  
				",10,3)";
...
String timetype=request.getParameter("timetype")==null?"time_id":request.getParameter("timetype");
         			...
         			<tr><td height="20" width="25%"  bgcolor="#EFF5FB" align="left">维度选择(单选)</td> 
         			 ...
         			 <tr>
							    ...
							    <select name="select1" size="6" id="select1" style="width:200">
							    	<option value="<%=timetype%>@timeid3" selected>统计时间</option>
							    	<%  
							    	    // 只能取出一个关键字段。
...
<div id="sqlview" style='display:<%if(((String)session.getAttribute("loginname")).equals("wangbotao")||((String)session.getAttribute("loginname")).equals("meikefu")) out.print("block"); else out.print("none");%>'></div>
