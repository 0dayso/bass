...
<head>
	...
	<script language="JavaScript" src="/hbbass/js/FusionCharts.js" charset=utf-8></script>
...
HashMap map = (HashMap)list.get(0);
	 String reportanalyquery=map.get("reportanalyquery").toString();
	 String report_name=map.get("report_name").toString();
	 //sqlca.execute(sql2);
	 //if(sqlca.next())
	 //{
	   ...
	   <tr>
	    <td colspan="1" valign="top">
         ...
         <table align="center" width="99%"  cellspacing="1" cellpadding="1" border="0">
					...
					conn = ConnectionManage.getInstance().getDWConnection();
	queryBase.setConnection(conn);
	sql = "select formtype,dataformat, formname, formid ,sqlreg ,tagname from NGBASS_REPORT_INPUT where pid="+pid+"  order by order with ur";
	list = (List)queryBase.query(sql);
					...
					String formIdStr=""; // 表单id组成的串
					 ...
					 while(i<list.size())
					 {
					   ...
					   formtype=tempMap.get("formtype").toString();
					   dataformat=tempMap.get("dataformat").toString();
					   formname=tempMap.get("formname").toString();
					   formid=tempMap.get("formid").toString();
					   sqlreg=tempMap.get("sqlreg").toString();
					    tagname=tempMap.get("tagname").toString();
         			...
         			<tr><td height="20" width="25%"  bgcolor="#EFF5FB" align="left">维度选择(单选)</td> 
         			 ...
         			 <tr>
							    ...
							    <select name="select1" size="6" id="select1" style="width:200">
					             ...
					             sql = "select   case when locate('=',sqlreg)>0 then lcase(replace(substr(sqlreg,1,locate('=',sqlreg)),'=',''))||'@'||value(tagname,'') "+
					             ...
					             {HashMap tempMap = (HashMap)list.get(i);
							    	%>
							        <option value="<%=tempMap.get("field")%>" <%if(i==0) out.println("selected");%>><%=tempMap.get("formname")%></option>
							      <%
							         i++;
							         }
							      %>  
							  ...
							  <tr>
							   ...
							   <tr> 
							    ...
							    <select name="select2" size="10" multiple="multiple" id="select2" style="width:200">
							    	...
							    	conn = ConnectionManage.getInstance().getDWConnection();
							    	queryBase.setConnection(conn);
							    	sql="select  VALUEREG||'@'||FIELDNAME VALUEREG, FIELDNAME   from NGBASS_REPORT_OUTput where   pid="+pid+"  order by ANALYSORDER with ur";
					             i=0;
					             list = (List)queryBase.query(sql);
					             while(i<list.size())
					             {HashMap tempMap = (HashMap)list.get(i);
							    	%>
							        <option value="<%=tempMap.get("VALUEREG")%>" <%if(i==0) out.println("selected");%>><%=tempMap.get("FIELDNAME")%></option>
							      <%
							         i++;
							        }
							      %>  
