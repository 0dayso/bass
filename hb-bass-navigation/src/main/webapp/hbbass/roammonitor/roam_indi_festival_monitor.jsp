...
function genSQL(){
	...
	if(date){
		...
		wherePart += " and (a.etl_cycle_id >= " + dateArr[0] + " ";
		wherePart += " and a.etl_cycle_id <= " + dateArr[1] + ") ";
	}
	if($('f001').value){
					...
					<div id="dim_div">
						<table align='center' width='99%' class='grid-tab-blue' cellspacing='1' cellpadding='0' border='0' style="display: ''">
							...
							<tr class='dim_row'>
								...
								<td class='dim_cell_content'>
									<select id="c001" name="c001" style="width: 250px" >
										<option value="">全部</option>
									</select>
								</td>
								<td class='dim_cell_title'>
...
window.onload=function(){
	...
	aihb.FormHelper.fillSelectWrapper({element:$('c001'),isHoldFirst:true,sql:"select feast_name||' ('||replace(char(start_date),'-','') || '-' || replace(char(end_date),'-','')||')' value, char(start_date) || '~' || char(end_date) key from nmk.dim_feast_day order by feast_year,start_date,end_date with ur"})
}
</script>