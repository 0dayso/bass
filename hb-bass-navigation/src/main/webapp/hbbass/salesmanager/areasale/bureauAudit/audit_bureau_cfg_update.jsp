						...
						<table align="center" width="99%">
							<tr class="dim_row_submit">
								<td align="center">
									...
									<input type="button" class="form_button" value="返回" onclick="backToList()">
								</td>
							</tr>
						</table>						
					</div>				
...
<script type="text/javascript">
function backToList(){
	window.location = '${mvcPath}/hbbass/salesmanager/areasale/bureauAudit/audit_bureau_cfg.jsp';
}
...
function save(){
	...
	var _ajax = new aihb.Ajax({
		url : "${mvcPath}/hbirs/action/areaSaleManage?method=updateAuditFlow"
	...
	//document.forms(0).action='${mvcPath}/hbirs/action/areaSaleManage?method=updateAuditFlow&cityId='+cityId+"&audit1="+audit1+"&audit2="+audit2;
	//document.forms(0).submit();
}
window.onload=function(){
	aihb.FormHelper.fillSelectWrapper({element:$('audit1'),isHoldFirst:true,sql:"select userid key,username||' <'||userid||'>'||' <'||mobilephone||'>' value from FPF_USER_USER where cityid='"+<%=city%>+"' order by userid with ur",ds:'web',isCache:false});
	aihb.FormHelper.fillSelectWrapper({element:$('audit2'),isHoldFirst:true,sql:"select userid key,username||' <'||userid||'>'||' <'||mobilephone||'>' value from FPF_USER_USER where cityid='"+<%=city%>+"' order by userid with ur",ds:'web',isCache:false});
};
