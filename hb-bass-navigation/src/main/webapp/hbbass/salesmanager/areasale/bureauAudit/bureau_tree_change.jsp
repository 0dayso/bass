...
var _header=[
	...
	,{"name":"ID","dataIndex":"c3"}
	,{"name":"ID名称","dataIndex":"c4"}
	,{"name":"等级","dataIndex":"c5"}
	//,{"name":"变更类型","dataIndex":"c6"}
	,{"name":"原父节点值","dataIndex":"c7"}
	,{"name":"原父节点名称","dataIndex":"c13"}
	,{"name":"层级","dataIndex":"c14"}
	,{"name":"新父节点值","dataIndex":"c8"}
	,{"name":"新父节点名称","dataIndex":"c8_2"}
...
function genSQL(){
	...
	if("toDo" == selType){
		...
		if($('level').value != null && $('level').value != ""){
			wherePiece += " and level="+$('level').value+"";
		}
		<%
		if(isProvince){//省
		   //hasRight= true;
		...
		var sql="select '' c0, CHANGETREE_SEQ c1, (select dim.itemname from mk.dim_areacity dim where dim.itemid=area_code) c2, ID c3, ID_NAME c4, (case when level=1 then '地市归属变更' when level=2 then '县域归属变更' when level=3 then '营销中心归属变更' when level=4 then '乡镇/街道归属变更' else '未知' end) c5, CHANG_TYPE c6, OLD_VALUE c7, NEW_VALUE c8,(select NAME from NWH.BUREAU_TREE where id=NEW_VALUE) c8_2, CREATEUSER c9,substr(char(CREATE_DATE),1,19) c9_2, case when STATUS='waitaudit1' then '待一级审核' when STATUS='waitaudit2' then '待二级审核' when STATUS='waitaudit3' then '待三级审核' when STATUS='succ' then '审核通过'  when STATUS='fail' then '审核不通过'  end c10, substr(char(STATE_DATE),1,19) c11,case when eff_type='0' then '立即生效' when eff_type='1' then '下月生效' end c12,(select NAME from NWH.BUREAU_TREE where id=old_value) c13,level c14 from BUREAU_TREE_CHANGE_INFO "
			+ wherePiece
			+ " and " + num + "=" + num
			+ getwhere('STATE_DATE') 
	...
	if("hasDone" == selType){
		...
		var sql="select '' c0, a.CHANGETREE_SEQ c1, (select dim.itemname from mk.dim_areacity dim where dim.itemid=area_code) c2, ID c3, ID_NAME c4, (case when level=1 then '地市归属变更' when level=2 then '县域归属变更' when level=3 then '营销中心归属变更' when level=4 then '乡镇/街道归属变更' else '未知' end) c5, CHANG_TYPE c6, OLD_VALUE c7, NEW_VALUE c8,(select NAME from NWH.BUREAU_TREE where id=NEW_VALUE) c8_2, CREATEUSER c9,substr(char(CREATE_DATE),1,19) c9_2, case when STATUS='waitaudit1' then '待一级审核' when STATUS='waitaudit2' then '待二级审核' when STATUS='waitaudit3' then '待三级审核' when STATUS='succ' then '审核通过'  when STATUS='fail' then '审核不通过'  end c10, substr(char(STATE_DATE),1,19) c11,case when eff_type='0' then '立即生效' when eff_type='1' then '下月生效' end c12 ,(select NAME from NWH.BUREAU_TREE where id=old_value) c13,level c14 from BUREAU_TREE_CHANGE_INFO a,BUREAU_TREE_CHANGE_Audit_log b where a.CHANGETREE_SEQ = b.CHANGETREE_SEQ and b.OPERATOR = '<%=userid%>' "
			+ wherePiece
			+ " and " + num + "=" + num
			+ getwhere('STATE_DATE') 
			+ getwhereDate('CREATE_DATE','date3','date4')	
					...
					<div id="dim_div">
						<table align='center' width='99%' class='grid-tab-blue' cellspacing='1' cellpadding='0' border='0'>
							<tr class='dim_row'>
								...
								<td class='dim_cell_title'>等级</td>
								<td class='dim_cell_content'>
									<select id="level">
										<option value="">全部</option>
										<option value="1">地市归属变更</option>
										<option value="12">地市名称变更</option>
										<option value="2">县域归属变更</option>
										<option value="22">县域名称变更</option>
										<option value="3">营销中心归属变更</option>
										<option value="32">营销中心名称变更</option>
										<option value="4">乡镇/街道归属变更</option>
										<option value="42">乡镇/街道名称变更</option>
									</select>
								</td>
							</tr>
							<tr class='dim_row'>
								<td class='dim_cell_title'>操作类型</td>
								<td class='dim_cell_content'><select id="CHANG_TYPE"><option value="">全部</option><option value="treepid" selected="selected">更改父节点</option></select></td>
...
<script type="text/javascript">
	function pass(){
		...
		var _ajax = new aihb.Ajax({
			url : "${mvcPath}/hbirs/action/areaSaleManage?method=bureauTreeChange"
	...
	function viewDetail(id){
		...
		var _url = "${mvcPath}/hbbass/salesmanager/areasale/bureauAudit/bureau_tree_change_detail.jsp?id="+ids;
