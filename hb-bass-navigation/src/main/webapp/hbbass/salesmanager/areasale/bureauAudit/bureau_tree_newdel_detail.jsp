...
try{
	...
	String sql = "select b.site_id from BUREAU_TREE_NEWDEL_INFO a,mk.dim_areacity b where a.AREA_code = b.itemid and a.NEWDELTREE_SEQ = "+id+"";
...
function genSQL(){
	var num = Math.random();	
	var	wherePiece = " where NEWDELTREE_SEQ = <%=id%>  and operate_type <> 'succ' ";
		
		var sql="select NEWDELTREE_SEQ c1, case when  operate_type='audit1' then '一级审核' when  operate_type='audit2' then '二级审核'  when  operate_type='audit3' then '三级审核' end c2, case  when  OPERATE_RESULT='succ' then '审核通过'  when  OPERATE_RESULT='fail' then '审核不通过'  end c3, substr(char(OPERATE_DATE),1,19) c4, (select username from FPF_USER_USER where userid=OPERATOR ) c5 from BUREAU_TREE_NEWDEL_AUDIT_LOG"
			+ wherePiece
			+ " and " + num + "=" + num
