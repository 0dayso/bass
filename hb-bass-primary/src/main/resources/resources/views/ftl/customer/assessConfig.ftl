...
<script type="text/javascript">
	...
	function desSql(){
		return strEncode(getSQL());
	}
	
	function getSQL(){
		var sql = "select id, city_id city, value(area_name,'省公司') area_name, case when type='1' then '营销案(事中预警)' when type='2' then '营销案(绩效评估预警)' when type='3' then '渠道监控预警'  when type='4' then '资费案预警' end type, username, mobilephone from customer_assess_config left join mk.bt_area on city_id = area_id where 1 = 1 ";
		if($("#city").val() !='' && $("#city").val() != null){
			sql = sql + " and city_id = " + $("#city").val();
		}
		if($("#type").val()!='' && $("#type").val()!=null){
			sql = sql + " and type = " + $("#type").val();
		}
		if($("#username").val()!='' && $("#username").val()!=null){
			sql = sql + " and username like '"+$("#username").val()+"'";
		}
		sql = sql + " order by create_dt desc ";
		return sql;
	}
	...
	function oper(val,options){
		...
		_aEdit.onclick=function(){
			$.ajax({
				type: "post"
				,url: "${mvcPath}/customer/deleteAssessConfig"
...
function add(elements){
	...
	iBtn.onclick=function(){
		...
		$.ajax({
			type: "post"
			,url: "${mvcPath}/customer/saveAssessConfig"
