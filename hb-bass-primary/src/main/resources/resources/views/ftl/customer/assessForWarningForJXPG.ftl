...
<script type="text/javascript">
	...
	function query(){
		...
		var grid = new aihb.CommonGrid({
			...
			,url:"${mvcPath}/customer/getAccessForWarningForJXPGInfo"
		});
		grid.run();
	}
	...
	function oper(val,options){
		...
		if (options.record.reply_level=='通过' || options.record.reply_level=='不通过'){
			...
			_aEdit.onclick=function(){
				...
				tabAdd({title:"营销案（事中预警）评估",id: mid,url:"${mvcPath}/customer/warningForAssessForJXPG?fee_id="+options.record.fee_id+"&time_id="+options.record.time_id+"&area_code="+options.record.area_code+"&type=2"});
			}
		}
