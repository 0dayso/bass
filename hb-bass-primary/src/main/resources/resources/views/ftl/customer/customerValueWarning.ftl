...
<script type="text/javascript">
	...
	function query(){
		...
		var grid = new aihb.CommonGrid({
			...
			,url:"${mvcPath}/customer/getCustValWarningList"
		});
		grid.run();
	}
	...
	function oper(val,options){
		...
		if (options.record.status=='已归档' || options.record.status=='待评估'){
			...
			_aEdit.onclick=function(){
				...
				tabAdd({title:"营销案（事中预警）",id: mid,url:"${mvcPath}/customer/replyForWarning?fee_id="+options.record.fee_id+"&time_id="+options.record.time_id+"&area_code="+options.record.area_code+"&type=1"});
				//window.location.href = "${mvcPath}/customer/replyForWarning?fee_id="+options.record.fee_id+"&time_id="+options.record.time_id+"&area_code="+options.record.area_code+"&type=1";
			};
		}
