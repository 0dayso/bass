...
<script type="text/javascript">
	...
	function view(){
		var time='${time_id}';
        var fee_id="channel_code$"+'${fee_id}';
		tabAdd({title:'客户价值渠道监控预警号码级清单明细',id: 16043,url:"${mvcPath}/report/16043?time="+time+"&condition="+fee_id});
	}
	...
	function query(){
		...
		var grid = new aihb.CommonGrid({
			...
			,url:"${mvcPath}/customer/getReplyForWarningForQDKZ"
		});
		grid.run();
	}
	...
	function save(){
		...
		$.ajax({
			...
			,data: "time_id="+time_id+"&area_code="+area_code+"&fee_id="+fee_id+"&fee_name="+fee_name+"&type=3&reply="+reply_content
			,dataType : "json"
