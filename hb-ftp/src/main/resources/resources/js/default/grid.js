						...
						,click : function(){
							...
							try{
								_grid.data.sort(function(_left,_right){
									...
									//增加不对全省汇总数据排序的功能
									if(_right.city=='总计'||_right.city=='合计'||_right.city=='全省'||_right.city=='湖北 '){
										return 1;
									}else if(_left.city=='总计'||_left.city=='合计'||_left.city=='全省'||_left.city=='湖北 '){
										return 1;
									}
									if(!isNaN(va1-va2)){//数值
...
aihb.AjaxHelper={
	...
	*/
	...
	down : function( options ){//form:表单, sql：sql语句,header:表头,fileName:下载的文件名,url:请求下载的地址,ds:数据源,useExcel:是否下载excel
		...
		//$(_form).attr("target","_top"); //由于下载需要金库验证，target=_top ，则直接跳出所有iframe框架，直接转向URL请求的链接页面，所以注释掉。
		_form.submit();
	},
...
function areacombo( level , sync ) {
	...
	if(document.forms[0].county){
		...
		sqls.push("select college_id key,college_name value from nwh.college_info where area_id='#{value}' order by college_name");
	}else if(document.forms[0].county_bureau){
		_elements.push(document.forms[0].county_bureau);
		sqls.push("select id key,name value from nwh.bureau_tree where area_code= '#{value}' and level=2 order by 1");
	...
	if(document.forms[0].office){
		...
		sqls.push("select id key,name value from nwh.bureau_tree where pid= '#{value}' order by 1");
	...
	if(document.forms[0].cell){
		...
		sqls.push("select bureau_id key,bureau_name name from nwh.dim_bureau_cfg where town_code='#{value}' order by 1");
...
aihb.Util = {
	...
	windowMask : function(options){
		var _container =$("<div>");
		_container.html('<div style="width:100%;height:100%;background:#252222;;position:fixed;z-index:99;left:0;top:0;filter:alpha(opacity=80);opacity: 0.5;">&#160;</div>');
		_container.hide();
		var _content=$("<div>",{style:"position:fixed;left:16%;top:5%;padding: 12px 15px 15px 15px;z-index:100;background-color: #EFF5FB;border:1px solid #c3daf9;letter-spacing: 2px;line-height: 28px;"});
...
function timelineTip(data){
	...
	if(run=='1' && check=='2'){
		...
		var tl_hold1=$("<div>",{id:"tl_hold1","class":"tl_banner",style:"cursor:pointer;font-size:12px;",text:"报表信息"});
		...
		.append(
			$("<li>",{style:"line-height:22px;padding-left:5px;color:#666;background-color:#fff;text-align:left"})
				.append($("<span>",{text:"预计更新时限:"})).append($("<span>",{text:timelineValue,style:"color:#FF5500;text-align:left"}))
		)
		.append(
			$("<li>",{style:"line-height:22px;padding-left:5px;color:#666;background-color:#fff;text-align:left"})
				.append($("<span>",{text:"程序:"})).append($("<span>",{text:value,style:"color:#FF5500;text-align:left"}))
		)
		.append(
			$("<li>",{style:"line-height:22px;padding-left:5px;color:#666;background-color:#fff;text-align:left"})
				.append($("<span>",{text:"批次:"})).append($("<span>",{text:cycle,style:"color:#FF5500;text-align:left"}))
		)
		var tl_hold2=$("<div>",{id:"tl_hold2","class":"tl_banner",style:"cursor:pointer;font-size:12px;",text:"运行情况",title:"点击展示"});
		tl_hold2.toggle(function () {
			tl_slide1[0].style.display = "";
	        $(this).attr('title','点击隐藏');
	    },function(){
	        tl_slide1[0].style.display = "none";
	         $(this).attr('title','点击展示');
	    });
		...
		.append(
			$("<li>",{style:"line-height:22px;padding-left:5px;color:#666;background-color:#fff;text-align:left"})
				.append($("<span>",{text:"程序状态:"})).append($("<span>",{text:status,style:"color:#FF5500;text-align:left"}))
		)
		.append(
			$("<li>",{style:"line-height:22px;padding-left:5px;color:#666;background-color:#fff;text-align:left"})
				.append($("<span>",{text:"报表负责人:"})).append($("<span>",{text:userId,style:"color:#FF5500;text-align:left"}))
		)
		.append(
			$("<li>",{style:"line-height:22px;padding-left:5px;color:#666;background-color:#fff;text-align:left"})
				.append($("<span>",{text:"程序负责人:"})).append($("<span>",{text:creater,style:"color:#FF5500;text-align:left"}))
		)
		.append(
			$("<li>",{style:"line-height:22px;padding-left:5px;color:#666;background-color:#fff;text-align:left"})
				.append($("<span>",{text:"运行开始时间:"})).append($("<span>",{text:begintime,style:"color:#FF5500;text-align:left"}))
		)
		.append(
			$("<li>",{style:"line-height:22px;padding-left:5px;color:#666;background-color:#fff;text-align:left"})
				.append($("<span>",{text:"运行完成时间:"})).append($("<span>",{text:endtime,style:"color:#FF5500;text-align:left"}))
		)
		.append(
			$("<li>",{style:"line-height:22px;padding-left:5px;color:#666;background-color:#fff;text-align:left"})
				.append($("<span>",{text:"平均运行时间:"})).append($("<span>",{text:runtime,style:"color:#FF5500;text-align:left"}))
		)
		var tl_hold3=$("<div>",{id:"tl_hold2","class":"tl_banner",style:"cursor:pointer;font-size:12px;",text:"分发情况",title:"点击展示"});
		...
		if(length>0){
			for(var i=0;i<length;i++){
				...
				.append(
					$("<li>",{style:"line-height:22px;padding-left:5px;color:#666;background-color:#fff;text-align:left"})
						.append($("<span>",{text:"分发接口号:"})).append($("<span>",{text:intercode,style:"color:#FF5500;text-align:left"}))
					)
				.append(
					$("<li>",{style:"line-height:22px;padding-left:5px;color:#666;background-color:#fff;text-align:left"})
						.append($("<span>",{text:"分发批次:"})).append($("<span>",{text:datecycle,style:"color:#FF5500;text-align:left"}))
				)
				.append(
					$("<li>",{style:"line-height:22px;padding-left:5px;color:#666;background-color:#fff;text-align:left"})
						.append($("<span>",{text:"导出状态:"})).append($("<span>",{text:exportstate,style:"color:#FF5500;text-align:left"}))
				)
				.append(
					$("<li>",{style:"line-height:22px;padding-left:5px;color:#666;background-color:#fff;text-align:left"})
						.append($("<span>",{text:"导入状态:"})).append($("<span>",{text:webstate,style:"color:#FF5500;text-align:left"}))
				)
				.append(
					$("<li>",{style:"line-height:22px;padding-left:5px;color:#666;background-color:#fff;text-align:left"})
						.append($("<span>",{text:"导出行数:"})).append($("<span>",{text:exportrows,style:"color:#FF5500;text-align:left"}))
				)
				.append(
					$("<li>",{style:"line-height:22px;padding-left:5px;color:#666;background-color:#fff;text-align:left"})
						.append($("<span>",{text:"导入行数:"})).append($("<span>",{text:webloadrows,style:"color:#FF5500;text-align:left"}))
				)
				.append(
					$("<li>",{style:"line-height:22px;padding-left:5px;color:#666;background-color:#fff;text-align:left"})
						.append($("<span>",{text:"导入时间:"})).append($("<span>",{text:web_time,style:"color:#FF5500;text-align:left"}))
				)
			}
		}
		...
		tl_slide.append(
			$("<li>",{style:"line-height:22px;padding-left:5px;color:#666;background-color:#fff;"})
				.append($("<span>",{text:"预计更新时限:"})).append($("<span>",{text:timelineValue,style:"color:#FF5500"}))
		)
	//	.append(
	//		$("<li>",{style:"line-height:22px;padding-left:5px;color:#666;background-color:#fff;"})
