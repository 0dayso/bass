...
function areacombo( level , sync ) {
	...
	if(document.forms[0].custmgr){
		...
		//sqls.push("select region_id key,region_name value from pt.kpi_bureau_cfg where parent_id= '#{value}' order by 1 with ur");
		sqls.push("select id key,name value from nwh.bureau_tree"+bureauSuffix+" where pid= '#{value}' order by 1");
...
aihb.Util = {
	...
	windowMask : function(options){
		...
		_close.html('<img src=\''+ mvcPath +'/resources/image/default/tab-close.gif\' style="cursor: hand;" onclick="{this.parentNode.parentNode.parentNode.style.display=\'none\';}"></img>');
