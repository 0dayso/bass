<BulletinSimple class="bulletin-simple">
	<div class="panel panel-primary">
		<div class="panel-heading bulletin-simple-title">
	      <span class="panel-title bulletin-simple-title-text">{opts.titleText}</span>
		  <a if={hasSubhead=='true'} class="panel-title icon-double-angle-down bulletin-simple-title-func" onclick="{clickTitleFunc}">
		  {opts.funcTitle}
		  </a>
		</div>
   		<div class="bulletin-simple-context">
			<ul class="nav nav-pills" each={row,idx in datas} if={idx < lineNum} onclick="{clickRow}">
				<li if(opts.itemIcon) class="{itemIcon} bulletin-simple-li-icon"></li>
	  			<li class="bulletin-simple-li-text" title={row.text}>{row.text}</li>
			</ul>
		</div>
	</div>
