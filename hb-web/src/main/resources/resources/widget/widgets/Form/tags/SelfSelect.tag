<SelfSelect class="SelfSelect">
	<span class="SelfSelect-name" if={opts.name}>{opts.name}:</span>
	<select class="SelfSelect-select" onchange={changeVal} value={nowPagenum}>
        <option each={obj,idx in opts.datas}  value ={obj.key}>{obj.value}</option>
    </select>
