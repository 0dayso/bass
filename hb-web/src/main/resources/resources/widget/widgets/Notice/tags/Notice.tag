<Notice class="notice">
		<i class=" icon-volume-up icon-2x pull-left icon-muted notice-icon"></i>
	    <span  class="notice-title">{opts.title}</span>
	    <label class="notice-text" style={opts.textStyle} title={opts.text} >{text}</label>
	    <a href={opts.href} class="notice-moretitle" onclick={opts.clickMore} style={opts.moreTitleStyle}>
	    	{opts.moreTitle}
	    </a>
   	var _this=this;
